...
function parseTimeAndRepeat(text) {
  ...
  if (lowerText.includes('her gün') || lowerText.includes('günlük')) {
    return parseDailyReminder(text);
  } else if (lowerText.includes('her hafta') || lowerText.includes('haftalık')) {
    return parseWeeklyReminder(text);
  } else if (lowerText.includes('her ay') || lowerText.includes('aylık')) {
    return parseMonthlyReminder(text);
  } else if (lowerText.includes('hafta içi') || lowerText.includes('iş günleri')) {
...
function parseDailyReminder(text) {
  ...
  // Metni temizle
  const cleanText = text.replace(/her gün|günlük|saat\s*\d{1,2}:\d{2}/gi, '').trim();
  ...
  return {
    ...
    timeText: `Her gün saat ${hour}:${minute.toString().padStart(2, '0')}`,
    repeat: 'daily',
    repeatText: 'Her gün',
